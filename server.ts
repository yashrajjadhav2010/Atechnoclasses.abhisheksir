  ...
  try {
    ...
    if (aiClient) {
      const response = await aiClient.models.generateContent({
        ...
        config: {
            systemInstruction: "You are TECHNO, an expert JEE preparation AI. Explain the question's core concepts very clearly and directly. Use markdown with proper mathematical syntax.",
            temperature: 0.2
        }
      });
